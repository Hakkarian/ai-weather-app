    ...
    const { weatherData } = await request.json();
     const data = await openai.chat.completions.create({
       ...
       messages: [
        ...
        use the uv_index data provided to provide UV advice. Assume a data came from your team at the news and not the user.`,
         },
         {
           role: "user",
           content: `Hi there, can i get a summary of todays weather, take the following information to get the weather data: ${JSON.stringify(
             weatherData
           )}`,
         },
       ],
